...
struct GradebookDiscipline: Codable, Identifiable, Equatable, Hashable {
    ...
    nonisolated var bestAttempt: GradeAttempt? {
        attempts.max(by: GradeAttempt.bestAttemptSort)
    }

    nonisolated var latestAttempt: GradeAttempt? {
        sortedAttempts.last
    }

    nonisolated var sortedAttempts: [GradeAttempt] {
        attempts.sorted { $0.attempt < $1.attempt }
    }

    nonisolated var bestGradeValue: Double? {
        bestAttempt?.grade.numericValue
    }

    nonisolated var averageGradeValue: Double? {
        let numericGrades = attempts.compactMap { $0.grade.numericValue }
        guard !numericGrades.isEmpty else { return nil }
        return numericGrades.reduce(0, +) / Double(numericGrades.count)
    }

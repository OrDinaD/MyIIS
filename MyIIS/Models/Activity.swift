...
extension Activity {
    struct PreviewConfig {
        ...
        let registrationDeadlineOffset: TimeInterval?
        let capacity: Int?
        let registered: Int
        let cancellationOffset: TimeInterval?
        ...
        init(
            ...
            self.registrationDeadlineOffset = registrationDeadlineOffset
            self.capacity = capacity
    ...
    static func preview(_ config: PreviewConfig) -> Activity {
        let now = Date()
        let start = now.addingTimeInterval(config.startOffset)
        let end = start.addingTimeInterval(config.duration)
        let deadline = config.registrationDeadlineOffset.map { start.addingTimeInterval($0) }
        let cancellationDate = config.cancellationOffset.map { start.addingTimeInterval($0) }
        ...
        return Activity(
            ...
            registeredCount: config.registered,
            cancellationDate: cancellationDate,
            userRegistration: config.registrationState
        )
    }

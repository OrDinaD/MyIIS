...
final class AnnouncementsService: AnnouncementsServicing {
    ...
    private static let cachePrefix = "AnnouncementsService.cache."
    ...
    func fetchCategories() async throws -> [AnnouncementCategory] {
        ...
        do {
            ...
            return try decodeCategories(from: data)
        } catch {
            if let cached = tryDecodeCachedCategories(for: request, originalError: error.localizedDescription) {
                return cached
            }
            throw error
        }
    ...
    func fetchAnnouncements(
        ...
        do {
            ...
            _ = try validate(response: response, data: data)
            persistCache(data: data, for: request)
            return try decodeAnnouncementPage(from: data)
        } catch {
            if let cached = tryDecodeCachedAnnouncementPage(for: request, originalError: error.localizedDescription) {
                return cached
            }
            throw error
        }
    ...
    private func decodeCategories(from data: Data) throws -> [AnnouncementCategory] {
    ...
    private func decodeAnnouncementPage(from data: Data) throws -> AnnouncementPage {
        let decoder = JSONDecoder()
    ...
    private func tryDecodeCachedCategories(for request: URLRequest, originalError: String) -> [AnnouncementCategory]? {
        guard let key = cacheKey(for: request),
              ...
              let decoded = try? decodeCategories(from: envelope.data) else {
            return nil
        }

        let url = request.url?.absoluteString ?? "—"
        logService.log("⚠️ Announcements categories offline cache used for \(url). Original error: \(originalError)")
        return decoded
    }

    private func tryDecodeCachedAnnouncementPage(for request: URLRequest, originalError: String) -> AnnouncementPage? {
        guard let key = cacheKey(for: request),
              let payload = UserDefaultsPayloadStore.load(forKey: key, from: userDefaults),
              let envelope = try? JSONDecoder().decode(CachedEnvelope.self, from: payload),
              let decoded = try? decodeAnnouncementPage(from: envelope.data) else {
            return nil
        }

        let url = request.url?.absoluteString ?? "—"
        logService.log("⚠️ Announcements offline cache used for \(url). Original error: \(originalError)")
        return decoded
    }

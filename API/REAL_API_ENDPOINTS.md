...
## Успеваемость — контракт от 26.09.2026

В предоставленном `Успеваемость.har` сайт вызывает `GET /api/v1/personal-rating`
без query-параметров и получает HTTP 200. В логе приложения прежний запрос личной
успеваемости возвращает HTTP 404. Авторизация — существующая сессия IIS.

Корень ответа: `subjects`, `deadlines`, `percentageMarks`.
Предмет: `id`, `abbrev`, `name`, `lessonTypes`.
Тип занятия: `id`, `abbrev`, `termHoursId`, `lessons`.
Занятие: `id`, `dateString`, `gradebookOmissions`, `subGroup`,
`marks: [{mark, taskNumber}]`, `controlPoint`.
Группы дедлайнов сопоставляются по `termHoursId`; содержат `taskCount`
и `deadlines`. Код сайта в HAR связывает дедлайн с датой занятия по `lessonId`.

Источник декодирования: `PersonalRatingModels.swift`. Общий рейтинг и журнал
старосты — отдельные функции. Остальные разделы ниже — исторические сведения,
не проверка всех эндпоинтов на указанную дату.

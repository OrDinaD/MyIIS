...
ka(b).then(e=>{t("EMBEDDED_MESSAGING_SET_CONVERSATION_DATA_EVENT",e)}))})})):(m("handlePrechatSubmit","Pre-chat submitted. Fetching a JWT and starting a new conversation."),Sa().then(c=>{t("ESW_SET_JWT_EVENT",c);ka(b).then(d=>{t("EMBEDDED_MESSAGING_SET_CONVERSATION_DATA_EVENT",d)})})))}function xb(){return new Promise(a=>{embeddedservice_bootstrap.createFilePreviewFrame();embeddedservice_bootstrap.createIframe().then(()=>{a("Created Embedded Messaging frame.")})})}function wb(){return q(H.JWT)?dc().then(a=>
{ta(a.accessToken);m("handleGetContinuityJwt","Sucessfully retreived a Continuity JWT");return a}).catch(a=>{l("handleGetContinuityJwt",`Failed to get Continuity JWT: ${a&&a.message?a.message:a}.`);W()}):Promise.reject(void 0)}function dc(){const a=embeddedservice_bootstrap.settings.scrt2URL.concat("/iamessage/v1/authorization/continuityAccessToken");return Y(a,"GET","cors",null,null,"getContinuityJwt").then(b=>{if(!b.ok)throw b;return b.json()})}function Sa(){return ec().then(a=>{ta(a.accessToken);
m("handleGetUnauthenticatedJwt","Successfully retreived an Unauthenticated JWT");return a}).catch(a=>fc(a))}function yb(a){var b=zb();b=b?embeddedservice_bootstrap.settings.scrt2URL.concat("/iamessage/v1/authorization/unauthenticated/accessToken","?",b):embeddedservice_bootstrap.settings.scrt2URL.concat("/iamessage/v1/authorization/unauthenticated/accessToken");return Y(b,"POST","cors",{"Content-Type":"application/json"},a,"sendUnauthenticatedJwtRequest").then(c=>{J()&&U();if(!c.ok)throw c;return c.json()})}
function ec(){let a={orgId:embeddedservice_bootstrap.settings.orgId,developerName:embeddedservice_bootstrap.settings.eswConfigDevName,capabilitiesVersion:"254"};return J()?Ab().then(b=>{if(!b)throw Error("ReCaptcha token generation failed at client.");a.captchaToken=b;return yb(a)}):yb(a)}function sb(){return(u()===y.EXP_SITE_AUTH?gc():hc()).then(a=>{a=u()===y.EXP_SITE_AUTH?a.data:a;ta(a.accessToken);m("handleGetAuthenticatedJwt","Successfully retreived an Authenticated JWT");return a}).catch(a=>
{J()&&U();a=(null==a?void 0:a.message)||a;l("handleGetAuthenticatedJwt",`Error retrieving authenticated token: ${a}.`);kb();throw a;})}function gc(){const a=embeddedservice_bootstrap.settings.snippetConfig.expSiteUrl+"/miaw/auth/accesstoken"+`?platformType=${"Web"}&developerName=${embeddedservice_bootstrap.settings.eswConfigDevName}&capabilitiesVersion=${"254"}`,b={Cos:"0x0204","Content-Type":"application/json"};return la(a,"GET","cors",b,null,"getExpSiteAuthenticatedJwt").then(c=>{if(!c.ok)throw c;
return c.json()}).catch(c=>za(c,a,"GET","cors",b,null,"getExpSiteAuthenticatedJwt"))}function Bb(a){const b=F,c=zb(),d=c?embeddedservice_bootstrap.settings.scrt2URL.concat("/iamessage/v1/authorization/authenticated/accessToken","?",c):embeddedservice_bootstrap.settings.scrt2URL.concat("/iamessage/v1/authorization/authenticated/accessToken"),e={"Content-Type":"application/json"};return b&&wa(b)?la(d,"POST","cors",e,a,"sendAuthenticatedJwtRequest").then(g=>{J()&&U();if(!g.ok)throw g;return g.json()}).catch(g=>
{J()&&U();return za(g,d,"POST","cors",e,a,"sendAuthenticatedJwtRequest")}):(new Promise(g=>{Cb({apiPath:d,method:"POST",mode:"cors",requestHeaders:e,requestBody:a,resolve:g})})).then(g=>{J()&&U();if(!g.ok)throw g;return g.json()})}function hc(){let a={orgId:embeddedservice_bootstrap.settings.orgId,developerName:embeddedservice_bootstrap.settings.eswConfigDevName,capabilitiesVersion:"254",authorizationType:Ta.JWT,customerIdentityToken:F};return J()?Ab().then(b=>{if(!b)throw Error("ReCaptcha token generation failed at client.");
a.captchaToken=b;return Bb(a)}):Bb(a)}function ic(){jc().then(()=>{m("handleRegisterDeviceCapabilities","Successfully Registered Device Capabilities.")})}function jc(){const a=embeddedservice_bootstrap.settings.scrt2URL.concat("/iamessage/v1/device/registerDeviceCapabilities");return Y(a,"POST","cors",null,{},"registerDeviceCapabilities")}function ya(a){return kc(!1,embeddedservice_bootstrap.settings.restrictSessionOnMessagingChannel?embeddedservice_bootstrap.settings.embeddedServiceConfig.embeddedServiceMessagingChannel.channelAddressIdentifier:
null).then(b=>{let c=[];if(!b.conversations||!Array.isArray(b.conversations))throw Error(`Invalid conversation list: ${b.conversations}.`);c=b.conversations.filter(d=>0===d.endTimestamp);if(0===c.length){if(a)throw r("handleListConversation","No open conversation found, deleting stale data from web storage.",!0),W(),Error("No open conversation found for JWT in web storage.");return tb()||(embeddedservice_bootstrap.settings.embeddedServiceConfig.termsAndConditions||{}).isTermsAndConditionsEnabled?
null:(r("handleListConversation","No existing conversation found and pre-chat is not enabled. Will start a new conversation."),ka(O).then(d=>d))}1<c.length&&(r("handleListConversation",`Expected the user to be participating in 1 open conversation but instead found ${c.length}. Loading the conversation with latest startTimestamp.`),c.sort((d,e)=>e.startTimestamp-d.startTimestamp));b=c[0];if(!lc(b.conversationId))throw Error(`Invalid conversation identifier: ${b.conversationId}.`);m("handleListConversation",
"Successfully retrieved existing conversation");A!==b.conversationId&&(m("handleListConversation",`ConversationId change detected. Old conversation-id: ${A} & New conversation-id: ${b.conversationId}`),A=b.conversationId);b.isExistingConversation=!0;return b}).catch(b=>{l("handleListConversation",`Failed to list conversation entries: ${b&&b.message?b.message:b}.`);throw Error(`Failed to list conversation entries: ${b&&b.message?b.message:b}.`);})}function kc(a,b){const c=embeddedservice_bootstrap.settings.scrt2URL.concat("/iamessage/v1/queries/conversation/list");
return Y(c,"POST","cors",null,Object.assign({},{includeClosedConversations:a},b&&{channelAddressIdentifier:b}),"listConversation").then(d=>d.json())}function fc(a){if(a&&a.status&&401===a.status)return l("handleGetJwtError","Unauthorized error caused by JWT expiration. Attempt to get another JWT",a.status),Sa().catch(b=>{throw Error(b);});J()&&U();throw Error(`Error retrieving unauthenticated token: ${(null==a?void 0:a.message)||a}.`);}function tb(){return(embeddedservice_bootstrap.settings.embeddedServiceConfig.forms||
[]).some(a=>{a.formFields=a.formFields||[];return"PreChat"===a.formType&&0<a.formFields.length})}function ka(a){if(Ia)return l("handleCreateNewConversation","Attempting to create new conversation multiple times!",null,!0),Promise.resolve();if(a)if("object"===typeof a)for(const [b,c]of Object.entries(a))m("handleCreateNewConversation",`Pre-chat field: ${b} ${c&&0<c.trim().length?`is set with a value of ${c.length} characters.`:"is not set."}`);else l("handleCreateNewConversation",`Expected to receive a valid object for Pre-Chat fields, instead received ${typeof a}`);
Ia=!0;return mc(a).then(b=>{z("onEmbeddedMessagingConversationStarted",{detail:{conversationId:A}});ic();b.esDeveloperName=embeddedservice_bootstrap.settings.eswConfigDevName;m("handleCreateNewConversation",`Initialized the esDeveloperName associated with current conversation to: ${embeddedservice_bootstrap.settings.eswConfigDevName}`);return b})}function mc(a){const b=embeddedservice_bootstrap.settings.scrt2URL.concat("/iamessage/v1/conversation");return Y(b,"POST","cors",null,Object.assign({},a&&
{routingAttributes:a},{conversationId:A,language:embeddedservice_bootstrap.settings.language}),"createNewConversation").then(c=>c.json())}function nc(){const a=embeddedservice_bootstrap.settings.scrt2URL.concat("/iamessage/v1/conversation","/",A,"/transcript");return Y(a,"GET","cors",null,null,"getTranscript").then(b=>b.arrayBuffer())}function $b(){z("onEmbeddedMessagingTranscriptRequested",{detail:{conversationId:A}});nc().then(a=>{a=new Blob([a],{type:"octet/stream"});a=window.URL.createObjectURL(a);
const b=document.createElement("a");b.href=a;b.download="ConversationTranscript.pdf";b.click();window.URL.revokeObjectURL(a);t("EMBEDDED_MESSAGING_REQUEST_TRANSCRIPT_SUCCESS_EVENT");z("onEmbeddedMessagingTranscriptDownloadSuccessful",{detail:{conversationId:A}})}).catch(a=>{if(a){const b=a.status;a=a.message||void 0;t("EMBEDDED_MESSAGING_REQUEST_TRANSCRIPT_ERROR_EVENT",{detail:{errorCode:b,errorMessage:a}});l("handleRequestTranscript",`Transcript request failed with status code: ${b} & message: ${a}`,
b)}else t("EMBEDDED_MESSAGING_REQUEST_TRANSCRIPT_ERROR_EVENT");z("onEmbeddedMessagingTranscriptRequestFailed",{detail:{conversationId:A}})})}function la(a,b,c,d,e,g=a){d=d?d:Object.assign({},{"Content-Type":"application/json"},M&&{Authorization:"Bearer "+M});e=e?JSON.stringify(e):void 0;const n=performance.now();return fetch(a,Object.assign({},{method:b,mode:c,headers:d},e&&{body:e})).then(k=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const v=((performance.now()-n)/1E3).toFixed(3);m("sendFetchRequest",
...
a&&a.status),la(b,c,d,e,g,n).catch(k=>{l("handleSendFetchRequestError",`${n} request failed again: ${k&&k.message?k.message:JSON.stringify(Object.assign({},k,{status:k.status,statusText:k.statusText,type:k.type}))}.`,k&&k.status);throw k;});if(401===a.status){l("handleSendFetchRequestError",`${n}: ${a&&a.message?a.message:JSON.stringify(Object.assign({},a,{status:a.status,statusText:a.statusText,type:a.type}))}.`,a&&a.status);if(u()===y.UNAUTH)throw K(),a;if(P())return F&&wa(F)||u()===y.EXP_SITE_AUTH?
new Promise(k=>{Db({apiPath:b,method:c,mode:d,requestHeaders:e,requestBody:g,resolve:k})}):new Promise(k=>{Cb({apiPath:b,method:c,mode:d,requestHeaders:e,requestBody:g,resolve:k})})}else if(423===a.status){const k={};Object.assign(k,{errorStatus:a.status,errorCode:a.errorCode});if(a.errorCode&&"ORG_UNDER_MAINTENANCE"===a.errorCode||"ORG_NOT_SUPPORTED"===a.errorCode)l("handleSendFetchRequestError",`Received a ${a.status} in ${n}: ${a&&a.message?a.message:JSON.stringify(Object.assign({},a,{status:a.status,
statusText:a.statusText,type:a.type}))}`,a&&a.status),V?t("EMBEDDED_MESSAGING_CONVO_ERROR_DATA_RECEIVED",k):ub(k)}else throw l("handleSendFetchRequestError",`Something went wrong in ${n}: ${a&&a.message?a.message:JSON.stringify(Object.assign({},a,{status:a.status,statusText:a.statusText,type:a.type}))}`,a&&a.status),a;}function Y(a,b,c,d,e,g){return la(a,b,c,d,e,g).catch(n=>za(n,a,b,c,d,e,g))}function zb(){let a="";const b=Ua();b&&(a=(new URLSearchParams({os:b.os.name,osVersion:b.os.version,browser:b.browser.name,
browserVersion:b.browser.version})).toString());return a}function Ua(){var a=[navigator.platform,navigator.userAgent,navigator.appVersion,navigator.vendor,window.opera].join(" ");const b=Eb(a,[{name:"Windows Phone",value:"Windows Phone",version:"OS"},{name:"Windows",value:"Win",version:"NT"},{name:"iPhone",value:"iPhone",version:"OS"},{name:"iPad",value:"iPad",version:"OS"},{name:"Kindle",value:"Silk",version:"Silk"},{name:"Android",value:"Android",version:"Android"},{name:"PlayBook",value:"PlayBook",
version:"OS"},{name:"BlackBerry",value:"BlackBerry",version:"/"},{name:"Macintosh",value:"Mac",version:"OS X"},{name:"Linux",value:"Linux",version:"rv"},{name:"Palm",value:"Palm",version:"PalmOS"}]);a=Eb(a,[{name:"Edge",value:"Edg",version:"Edg"},{name:"Chrome",value:"Chrome",version:"Chrome"},{name:"Firefox",value:"Firefox",version:"Firefox"},{name:"Safari",value:"Safari",version:"Version"},{name:"Internet Explorer",value:"MSIE",version:"MSIE"},{name:"Opera",value:"Opera",version:"Opera"},{name:"BlackBerry",
value:"CLDC",version:"CLDC"},{name:"Mozilla",value:"Mozilla",version:"Mozilla"}]);return{os:b,browser:a}}function Eb(a,b){let c;let d;for(c=0;c<b.length;c+=1){var e=new RegExp(b[c].value,"i");if(e=e.test(a)){e=new RegExp(b[c].version+"[- /:;]([\\d._]+)","i");e=a.match(e);d="";e&&e[1]&&(e=e[1]);if(e)for(e=e.split(/[._]+/),a=0;a<e.length;a+=1)d=0===a?d+(e[a]+"."):d+e[a];else d="0";return{name:b[c].name,version:parseFloat(d)}}}return{name:"unknown",version:0}}function I(){try{return embeddedservice_bootstrap.settings.siteURL}catch(a){l("getSiteURL",
`Error getting Site URL: ${a}`)}}function D(a,b){for(const c of embeddedservice_bootstrap.settings.embeddedServiceConfig.customLabels)if(c.sectionName===a&&c.labelName===b)return c.labelValue||"";for(const c of embeddedservice_bootstrap.settings.standardLabels)if(c.sectionName===a&&c.labelName===b)return c.labelValue||"";return""}function T(a){for(const b of embeddedservice_bootstrap.brandingData)if(b.n&&b.n===a)return b.v}function oc(){const a=embeddedservice_bootstrap.settings.chatButtonPosition;
if("string"===typeof a&&0<a.length){const b=a.split(",");2!==b.length?l("setButtonPositionStyles",`Invalid embeddedservice_bootstrap.settings.chatButtonPosition value specified: ${a}. Valid format is "25px, 30px".`):(document.documentElement.style.setProperty("--eswButtonBottom",b[0].trim()),document.documentElement.style.setProperty("--eswButtonRight",b[1].trim()))}}function Fb(){let a=B(),b=document.getElementById("embeddedMessagingIconContainer"),c=document.getElementById("embeddedMessagingIconChat"),
d=document.createElement("div"),e=document.createElement("span"),g,n=1;if(a){c.style.display="none";d.setAttribute("class","embeddedMessagingLoadingSpinner");for(d.setAttribute("id","embeddedMessagingLoadingSpinner");13>n;n++)g=document.createElement("div"),g.setAttribute("class","embeddedMessagingLoadingCircle"+n+" embeddedMessagingLoadingCircle"),d.appendChild(g);d.classList.add("embeddedMessagingIconLoading");a.classList.add("embeddedMessagingConversationButtonLoading");b.insertBefore(d,c);a.disabled=
!0;e.className="slds-assistive-text";e.setAttribute("role","alert");e.innerHTML=D("EmbeddedMessagingPrechat","PrechatSubmitButtonLoading")||"Loading";d.appendChild(e)}}function Va(){return-1===navigator.userAgent.indexOf("Mobi")}function va(){if(!Q)return!0;const a=Number(Q.startTime),b=Number(Q.endTime),c=Date.now();return c>=a&&c<b?!0:!1}function Pa(){let a;if(Q){var b=va();b?a=Number(Q.endTime):(a=Number(Q.startTime),z("onEmbeddedMessagingBusinessHoursEnded"));isNaN(a)||(ma&&clearTimeout(ma),ma=
setTimeout(()=>{clearTimeout(ma);ma=void 0;pc(b);b?z("onEmbeddedMessagingBusinessHoursEnded"):z("onEmbeddedMessagingBusinessHoursStarted")},a-Date.now()))}}function pc(a){a?(Q=void 0,embeddedservice_bootstrap.utilAPI.hideChatButton(),Oa().then(Pa()).catch(b=>rb(b))):(na()||embeddedservice_bootstrap.utilAPI.showChatButton(),Pa())}function qc(){const a=!!navigator.userAgent.match(/iP(hone|ad|od)/i);var b=navigator.userAgent.match(/(?!=OS)(([0-9]){2})/i);b=b&&0<b.length?Number(b[0]):-1;const c=!!navigator.userAgent.match(/WebKit/i)&&
!navigator.userAgent.match(/CriOS/i);return a&&c&&15<=b}function na(){try{return embedded_svc&&embedded_svc.menu?!0:!1}catch(a){return!1}}function lb(){return navigator.userAgent.includes("CommunityHybridContainer")||navigator.userAgent.includes("playgroundcommunity")}function Ba(){return embeddedservice_bootstrap.settings.snippetConfig.expSiteContext===Qa.LWR||embeddedservice_bootstrap.settings.snippetConfig.expSiteContext===Qa.AURA}function Ca(){return embeddedservice_bootstrap.settings.displayMode===
Gb.INLINE}function Wa(a){const b=embeddedservice_bootstrap.settings.targetElement.children;let c;for(const d in b)c=b[d],c!==S()&&c!==embeddedservice_bootstrap.filePreviewFrame&&(c.inert=a)}function ac(a){const b=document.getElementById("embeddedMessagingLiveRegion");b.textContent=a.text;Xa&&clearTimeout(Xa);Xa=setTimeout(()=>{b.textContent=""},3E3)}function Hb(){const a=document.querySelector('meta[name="viewport"]');a&&(a.setAttribute("content",Z),Z=void 0)}function t(a,b){Ga.then(()=>{const c=
...
c&&c.contentWindow?c.contentWindow.postMessage(Object.assign({},{method:a},b&&{data:b}),I()):r("sendPostMessageToSiteContextIframe",`Embedded Messaging site context iframe not available for post message with method ${a}.`)})}function Ib(){ba("ESW_PREFETCH_LWR_RESOURCES")}function sc(a){a||l("handleAuraSite","Failed to load aura app. Iframe is undefined.");a.src=I()+"/embeddedService/embeddedService.app"}function tc(a){let b=I();a||l("handleLWRSite","Failed to load LWR site. Iframe is undefined.");
b.endsWith("/")||(b+="/");a.src=b+"?lwc.mode="+(embeddedservice_bootstrap.settings.devMode?"dev":"prod")}function nb(){return new Promise((a,b)=>{try{let c=B(),d=embeddedservice_bootstrap.utilAPI.getEmbeddedMessagingFrame();if(c&&!c.classList.contains("embeddedMessagingConversationButtonLoaded")){if(X())return m("handleClick","Conversation button clicked but a JWT exists in web storage. Joining the existing conversation."),M=q(H.JWT),Jb(!1).then(()=>{a()}).catch(e=>{N();l("handleClick",e.message);
b(e)});E(H.CONVERSATION_BUTTON_CLICK_TIME,Date.now(),!0,!0);Fb();m("handleClick","Conversation button clicked");xb().then(()=>{a()}).catch(e=>{l("handleClick",e.message);b(e)});(embeddedservice_bootstrap.settings.embeddedServiceConfig.fallbackMessage||{}).isFallbackMessageEnabled?ub():bc().catch(e=>{N();l("handleClick",e.message);b(e)})}else c&&c.classList.contains("embeddedMessagingConversationButtonLoaded")&&d&&d.classList&&d.classList.contains("isMaximized")?t("ESW_APP_MINIMIZE"):l("handleClick",
"Something went wrong handling button click event.")}catch(c){b(c)}})}function uc(a){let b=embeddedservice_bootstrap.utilAPI.getEmbeddedMessagingFrame();a&&a.key&&(a.key===oa.SPACE||a.key===oa.ENTER?(a.preventDefault(),mb()):a.key===oa.TAB&&a.shiftKey&&b&&b.classList&&b.classList.contains("isMaximized")&&(a.preventDefault(),t("trapfocustolast")))}function vc(){const a=document.getElementById("embeddedMessagingMinimizedNotification"),b=B();a&&a.parentNode.removeChild(a);b&&b.focus()}function wc(a){if(a.key===
oa.ENTER||a.key===oa.SPACE)a.preventDefault(),this.handleNotificationDimissButtonClick()}function X(){return!!q(H.JWT)}function Jb(a){if(q(H.JWT))return Fb(),xb().catch(l),wb().then(b=>{ya(a).then(c=>{ja(b,c,null,a)})}).catch(l)}function W(a){m("resetClientToInitialState","Resetting client to initial state.");try{K(a),Ub(),p()}catch(b){l("resetClientToInitialState",`Error on clearing web storage for the previously ended conversation: ${b}`)}Z&&Hb();Ya.test(navigator.userAgent)&&Wa(!1);embeddedservice_bootstrap.removeMarkup();
Kb()?Lb().then(()=>Mb()):Mb()}function Mb(){if(u()===y.UNAUTH||u()===y.EXP_SITE_AUTH&&!embeddedservice_bootstrap.settings.snippetConfig.isExpSiteGuestUser)u()===y.EXP_SITE_AUTH&&xa(),embeddedservice_bootstrap.generateMarkup(),B()&&B().focus();m("resetClientToInitialState","Successfully reset client to initial state.");pa&&"function"===typeof pa&&(pa(),m("resolveClearSessionPromise","Resolved Clear Session promise."),pa=void 0)}function xc(){const a=document.createElement("div");a.id="embeddedMessagingModalOverlay";
...
-1);a.style.setProperty("cursor","default");a.classList.add("no-hover");c&&b&&c.removeChild(b);J()&&U();b=La(Ea.REFRESH);b.setAttribute("id","embeddedMessagingIconRefresh");b.setAttribute("class","embeddedMessagingIconRefresh");c.appendChild(b);c=document.createElement("div");b=document.createElement("div");const e=document.createElement("span"),g=document.createElement("span");var d=document.createElement("button");const n=document.createElement("span"),k=document.createElement("span");d.className=
"embeddedMessagingMinimizedNotificationDismissButton";d.addEventListener("click",vc);d.addEventListener("keydown",wc);d.setAttribute("aria-describedby","dismissButton-help");n.className="embeddedMessagingMinimizedNotificationDismissButtonText";n.title=D("EmbeddedMessagingMinimizedState","NotificationDismissButtonText")||"Dismiss";n.innerHTML=D("EmbeddedMessagingMinimizedState","NotificationDismissButtonText")||"Dismiss";n.style.setProperty("font-family",T("font"));k.className="slds-assistive-text";
k.id="dismissButton-help";k.innerHTML=D("EmbeddedMessagingMinimizedState","MinimizedNotificationDismissButtonAssistiveText")||"Close the chat notification";d.appendChild(n);d.appendChild(k);c.id="embeddedMessagingMinimizedNotification";c.className="embeddedMessagingMinimizedNotification";b.className="embeddedMessagingMinimizedNotificationTextWrapper";e.className="embeddedMessagingMinimizedNotificationText";e.role="status";e.title=D("EmbeddedMessagingMinimizedState","JWTRetrievalFailureText")||"Something went wrong. Log in again to continue your messaging conversation.";
e.innerHTML=D("EmbeddedMessagingMinimizedState","JWTRetrievalFailureText")||"Something went wrong. Log in again to continue your messaging conversation.";e.style.setProperty("font-family",T("font"));g.className="slds-assistive-text";g.innerHTML=D("EmbeddedMessagingMinimizedState","MinimizedNotificationAssistiveText")||"Expand the text.";b.appendChild(e);b.appendChild(g);c.appendChild(b);c.appendChild(d);a.disabled=!0;embeddedservice_bootstrap.settings.targetElement.appendChild(c)}}function Ob(a){a=
...
b.exp)).toString()}`),c}catch(d){return embeddedservice_bootstrap.settings.devMode&&l("validateJwt",`JWT validation failed: ${d.message}`),!1}}function u(){let a=null;try{a=embeddedservice_bootstrap.settings.embeddedServiceConfig&&embeddedservice_bootstrap.settings.embeddedServiceConfig.embeddedServiceMessagingChannel&&embeddedservice_bootstrap.settings.embeddedServiceConfig.embeddedServiceMessagingChannel.authMode}catch(b){l("getAuthMode",`Failed to retrieve auth mode flag: ${b.message}`)}return a}
function P(){return u()===y.AUTH||u()===y.EXP_SITE_AUTH}function ra(){}function Pb(a,b,c,d){const e="setHiddenPrechatFields"===a;var g=e?yc():aa();const n=g.map(({name:k})=>k);g=g.find(k=>k.name===b);return n.includes(b)?"string"===typeof c&&(c.toLowerCase().includes("javascript:")||c.toLowerCase().includes("<script>"))?(l("validatePrechatField",`JavaScript isn't allowed in the value for the ${b} field when calling ${a}.`),!1):c&&String(c).length>g.maxLength?(l("validatePrechatField",`Value for the ${b} field in ${a} exceeds the maximum length restriction of ${g.maxLength} characters.`),
!1):e&&"string"!==typeof c?(l("validatePrechatField",`You must specify a string for the ${b} field in ${a} instead of a ${typeof c} value.`),!1):d&&"boolean"!==typeof d?(l("validatePrechatField","setVisiblePrechatFields was called with isEditableByEndUser set to a value that\u2019s not boolean. Enter a boolean value."),!1):!0:(l("validatePrechatField",`${a} called with an invalid field name ${b}.`),!1)}function yc(){let a=[];var b=Array.isArray(embeddedservice_bootstrap.settings.embeddedServiceConfig.forms)&&
embeddedservice_bootstrap.settings.embeddedServiceConfig.forms.length&&Array.isArray(embeddedservice_bootstrap.settings.embeddedServiceConfig.forms[0].hiddenFormFields)?!0:!1;b&&(a=embeddedservice_bootstrap.settings.embeddedServiceConfig.forms[0].hiddenFormFields);return a}function aa(){return Array.isArray(embeddedservice_bootstrap.settings.embeddedServiceConfig.forms)&&embeddedservice_bootstrap.settings.embeddedServiceConfig.forms.length&&Array.isArray(embeddedservice_bootstrap.settings.embeddedServiceConfig.forms[0].formFields)&&
embeddedservice_bootstrap.settings.embeddedServiceConfig.forms[0].formFields.length?embeddedservice_bootstrap.settings.embeddedServiceConfig.forms[0].formFields:[]}function Fa(a){return L?!0:(l("shouldProcessPrechatFieldsFromHost",`Can't call ${a} before the onEmbeddedMessagingReady event is fired.`),!1)}function lc(a){return"string"===typeof a&&0<a.trim().length?!0:!1}function Vb(){aa().forEach(a=>{delete a.value;delete a.isEditableByEndUser})}function $a(){}function Qb(a){return L?!0:(l("shouldProcessAutoResponseParametersFromHost",
`[${a}] Cannot invoke Auto-Response API before the onEmbeddedMessagingReady event is fired.`),!1)}function R(){}function C(){this.settings={devMode:!1,targetElement:document.body};this.isSessionStorageAvailable=this.isLocalStorageAvailable=!0;Object.defineProperties(Ea,{CHAT:{value:"M50 0c27.614 0 50 20.52 50 45.833S77.614 91.667 50 91.667c-8.458 0-16.425-1.925-23.409-5.323-13.33 6.973-21.083 9.839-23.258 8.595-2.064-1.18.114-8.436 6.534-21.767C3.667 65.54 0 56.08 0 45.833 0 20.52 22.386 0 50 0zm4.583 61.667H22.917a2.917 2.917 0 000 5.833h31.666a2.917 2.917 0 000-5.833zm12.5-15.834H22.917a2.917 2.917 0 000 5.834h44.166a2.917 2.917 0 000-5.834zM79.583 30H22.917a2.917 2.917 0 000 5.833h56.666a2.917 2.917 0 000-5.833z"},
...
parseInt(a.slice(3,5),16);a=parseInt(a.slice(5,7),16);return{r:b,g:c,b:a}}}const Ac=".salesforce.com .force.com .sfdc.net .site.com .salesforce-sites.com .crm.dev .crmforce.mil .salesforce.mil".split(" "),Ta={JWT:"JWT"},ob=document.title;let Rb,ia,Sb=!1,L=!1,V=!1,da=!1,Ja=!1,Ia=!1,O={},ca={},M,F,x,A,Ha,Ga=new Promise(a=>{Ha=a}),hb;const rc=new Promise(a=>{hb=a});let ib,Bc=new Promise(a=>{ib=a}),Z,Q,ma,Ra,Xa;const H={JWT:"JWT",FAILED_OUTBOUND_MESSAGE_ENTRIES:"FAILED_MESSAGES",CONVERSATION_BUTTON_CLICK_TIME:"CONVERSATION_BUTTON_CLICK_TIME"},
...
Xb();oc();embeddedservice_bootstrap.settings.hasBottomTabBar=qc();embeddedservice_bootstrap.settings.isAuraSite=!!embeddedservice_bootstrap.settings.isAuraSite;embeddedservice_bootstrap.settings.omitSandbox=!!embeddedservice_bootstrap.settings.omitSandbox;embeddedservice_bootstrap.settings.restrictSessionOnMessagingChannel=!!embeddedservice_bootstrap.settings.restrictSessionOnMessagingChannel;embeddedservice_bootstrap.settings.shouldMinimizeWindowOnNewTab=!!embeddedservice_bootstrap.settings.shouldMinimizeWindowOnNewTab;

...
final class AttendanceViewModelTests: XCTestCase {
    ...
    @MainActor func testMonthParser_LocalizedTitle_UsesRequestedLocale() {
        XCTAssertEqual(
            MonthParser.localizedTitle(from: "Сентябрь", locale: Locale(identifier: "be_BY")),
            "Верасень"
        )
        XCTAssertEqual(
            MonthParser.localizedTitle(from: "Октябрь 2026 г.", locale: Locale(identifier: "en_US")),
            "October"
        )
    }

    @MainActor func testMonthParser_LocalizedTitle_PreservesUnknownValue() {
        XCTAssertEqual(
            MonthParser.localizedTitle(from: "Учебный месяц", locale: Locale(identifier: "be_BY")),
            "Учебный месяц"
        )
    }
